        ...
        __device__ __forceinline__ void Store(){
            if(warp_id_ < active_warp_num_){
                ...
                if(intra_warp_tid_ < active_thread_num_){
                    *(output_values_base_ + (intra_warp_tid_ % half_active_thread_num_) * 2 + intra_warp_tid_ / half_active_thread_num_ + warp_id_ * output_warp_width_) = __float2half((float)(output_fragment_[0]) / scale_);
                }
            }
        }
    };
